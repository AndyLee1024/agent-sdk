...
from terminal_agent.app import (
    _compute_visual_line_ranges,
    _index_for_visual_col,
    _read_env_int,
    _visual_col_for_index,
)
...
class TestInputBehaviorHelpers(unittest.TestCase):
    def test_compute_visual_line_ranges_wraps_ascii(self) -> None:
        text = "abcdefghij"
        ranges = _compute_visual_line_ranges(text, max_cols=4)
    ...
    def test_compute_visual_line_ranges_handles_wide_chars(self) -> None:
        text = "你a你"
        ranges = _compute_visual_line_ranges(text, max_cols=3)
    ...
    def test_visual_col_and_index_roundtrip(self) -> None:
        ...
        for idx in range(start, end + 1):
            col = _visual_col_for_index(text, start, end, max_cols, idx)
            roundtrip = _index_for_visual_col(text, start, end, max_cols, col)
            self.assertLessEqual(roundtrip, idx)

    def test_read_env_int_defaults_and_validation(self) -> None:
        name = "AGENT_SDK_TEST_ENV_INT"
        os.environ.pop(name, None)
        self.assertEqual(_read_env_int(name, 123), 123)

        os.environ[name] = "456"
        self.assertEqual(_read_env_int(name, 123), 456)
