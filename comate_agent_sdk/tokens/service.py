...
from comate_agent_sdk.tokens.custom_pricing import CUSTOM_MODEL_PRICING
from comate_agent_sdk.tokens.mappings import MODEL_TO_LITELLM
from comate_agent_sdk.tokens.views import (
    CachedPricingData,
    ModelPricing,
    ModelUsageStats,
    ModelUsageTokens,
...
class TokenCost:
    ...
    CACHE_DIR_NAME = "comate_agent_sdk/token_cost"
    CACHE_DURATION = timedelta(days=1)
    PRICING_URL = "https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json"

    def __init__(self, include_cost: bool = False):
        env_val = os.getenv("comate_agent_sdk_CALCULATE_COST")
    ...
    def _find_model_in_pricing_data(self, model_name: str) -> dict | None:
        ...
        prefixes = ["anthropic/", "openai/", "google/", "azure/", "bedrock/", "gemini/"]
    ...
    async def get_usage_summary(
        ...
        source_stats: dict[str, ModelUsageStats] = {}
        ...
        for entry in filtered_usage:
            ...
            if entry.level:
                ...
                ls.total_tokens += entry.usage.total_tokens
                ls.invocations += 1

            source = entry.source or "unknown"
            if source not in source_stats:
                source_stats[source] = ModelUsageStats(model=source)
            ss = source_stats[source]
            ss.prompt_tokens += entry.usage.prompt_tokens
            ss.completion_tokens += entry.usage.completion_tokens
            ss.total_tokens += entry.usage.total_tokens
            ss.invocations += 1
            ...
            if self.include_cost:
                ...
                if cost:
                    ...
                    source_stats[source].cost += cost.total_cost
