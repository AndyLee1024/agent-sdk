...
from comate_agent_sdk.mcp.sdk_tool import is_sdk_mcp_tool
from comate_agent_sdk.mcp.types import McpSdkServerConfig, SdkMcpTool

logger = logging.getLogger("comate_agent_sdk.mcp.sdk_server")
...
def create_sdk_mcp_server(
    *,
    ...
    # 记录版本信息（仅用于调试/元数据）
    setattr(server, "_comate_agent_sdk_version", str(version))
    ...
    if tools:
        for fn in tools:
            ...
            tool_name = getattr(fn, "_comate_agent_sdk_mcp_tool_name", None)
            tool_desc = getattr(fn, "_comate_agent_sdk_mcp_tool_description", None)
