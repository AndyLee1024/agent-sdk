...
from comate_agent_sdk.system_tools.tools import LS, MultiEdit, TodoWrite, WebFetch
from comate_agent_sdk.tools.system_context import bind_system_tool_context
from comate_agent_sdk.tokens import TokenCost

logger = logging.getLogger("comate_agent_sdk.examples.system_tools_smoke_direct")


async def main() -> None:
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s")

    session_id = os.getenv("COMATE_AGENT_SDK_SMOKE_SESSION_ID") or f"smoke-{uuid.uuid4().hex[:8]}"
    ...
    with bind_system_tool_context(
        ...
        tmp_dir = Path("/tmp") / "comate_agent_sdk_smoke"
        ...
        # LS: list /tmp/comate_agent_sdk_smoke
